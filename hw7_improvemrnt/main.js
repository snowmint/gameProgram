...
//    agent = new Agent(new THREE.Vector3(-400 + 400 * Math.random(), 0, -400 + 400 * Math.random()), mesh);
    ...
    agentB = new Agent("BlueArrow", new THREE.Vector3(getRandom(-400, 800), 0, getRandom(-400, 800)), size, 0x69a8ff);
    agentR = new Agent("RedArrow", new THREE.Vector3(getRandom(-400, 800), 0, getRandom(-400, 800)), size, 0xff6a88);
...
function animate() {
  ...
  agentB.update(0.01, agentR)
  agentR.update(0.01, agentB)
  ...
  if (scene.targets.length > 0) {
    ...
    updateScoreBoard(agentB, agentR);
    counting = false;
  	alert ('                                            << game over >>' + '\n\n                                  Time Consuming : ' + minute + ' : ' + second + ' : ' + msecond + '\n\n                                 ' + agentB.name + " : " + agentB.score.toFixed(1) + '      ' + agentR.name + ' : ' + agentR.score.toFixed(1));
  }
  render();
  updateScoreBoard(agentB, agentR);
}
